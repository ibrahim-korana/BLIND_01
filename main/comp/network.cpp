...
bool Network::ap_init(void)
{
    ...
    wifi_ap_config_t aa = {};
    strcpy((char*)aa.ssid, "SMQ_DALI");
    strcpy((char*)aa.password, "12345678");
    aa.ssid_len = strlen("SMQ_DALI");
...
bool Network::sta_init(void)
{
    ...
    ESP_LOGI("WIFI STA","SSID  : %s",aa.ssid);
    ESP_LOGI("WIFI STA","PASSW : %s",aa.password);
...
esp_err_t Network::ap_connect(void)
{
    if (!net_init()) return ESP_FAIL;
